...
export class MobDebugSession extends LoggingDebugSession {
	...
	public constructor(fileAccessor: FileAccessor) {
		...
		this._runtime.on('stopOnEntry', () => {
			this.sendEvent(new StoppedEvent('entry', MobDebugSession.threadID));
		});
		this._runtime.on('stopOnStep', () => {
			this.sendEvent(new StoppedEvent('step', MobDebugSession.threadID));
		});
		this._runtime.on('stopOnBreakpoint', () => {
			this.sendEvent(new StoppedEvent('breakpoint', MobDebugSession.threadID));
		});
		this._runtime.on('stopOnDataBreakpoint', () => {
			this.sendEvent(new StoppedEvent('data breakpoint', MobDebugSession.threadID));
		});
		this._runtime.on('stopOnInstructionBreakpoint', () => {
			this.sendEvent(new StoppedEvent('instruction breakpoint', MobDebugSession.threadID));
		});
		this._runtime.on('stopOnException', (exception) => {
			if (exception) {
				this.sendEvent(new StoppedEvent(`exception(${exception})`, MobDebugSession.threadID));
			} else {
				this.sendEvent(new StoppedEvent('exception', MobDebugSession.threadID));
			}
		});
	...
	protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
		...
		response.body = {
			threads: [
				new Thread(MobDebugSession.threadID, "thread 1"),
				new Thread(MobDebugSession.threadID + 1, "thread 2"),
			]
		};
		this.sendResponse(response);
	}
